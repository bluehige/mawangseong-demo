...
func _check_data_schema() -> void:
	...
	_expect(_has_grid_cell(room_grid, "G01_02", "slot_01"), "grid cell G01_02 anchors build slot")
	_expect(_has_grid_cell(room_grid, "G02_02", "watch_post"), "grid cell G02_02 anchors watch post")
...
func _check_game_root_integration() -> void:
	...
	_expect(game.quarter_renderer.debug_wall_cell_count() == 14, "quarter renderer emits 14 de-duplicated closed wall edges for complete 3x3 layout")
	_expect(game.quarter_renderer.debug_wall_edge_key_for_cell(Vector2i(8, 9), "N").begins_with("wall_N_"), "blocked entrance north edge resolves to a north wall")
	_expect(game.quarter_renderer.debug_wall_edge_key_for_cell(Vector2i(9, 9), "N").begins_with("wall_N_"), "blocked spike corridor north edge resolves to a north wall")
	_expect(game.quarter_renderer.debug_wall_edge_key_for_cell(Vector2i(8, 8), "S") == "", "shared blocked throne south edge is de-duplicated to entrance north")
	_expect(game.quarter_renderer.debug_wall_edge_key_for_cell(Vector2i(8, 9), "E") == "", "connected entrance east edge does not resolve to a closed wall")
	...
	_expect(game.quarter_renderer.debug_object_facing("throne") == "SW", "throne object facing is explicit SW")
	_expect(game.quarter_renderer.debug_object_texture_key("throne", "front") == "prop:throne_f:SW:front", "throne uses SW-facing v3 front sprite")
	_expect(game.quarter_renderer.debug_object_texture_key("barracks", "back") == "prop:weapon_rack:SW:back", "barracks uses SW-facing v3 back sprite")
	_expect(game.graph.path_between("entrance", "watch_post").size() >= 4, "complete default layout keeps watch post branch path")
	_expect(game.quarter_renderer.debug_object_texture_key("watch_post", "front") == "prop:watch_post:SW:front", "watch post uses SW-facing v3 front sprite")
