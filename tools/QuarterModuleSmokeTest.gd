...
func _check_game_root_integration() -> void:
	...
	_expect(game.quarter_renderer.has_module_visuals(), "quarter module visual renderer has textures")
	_expect(game.quarter_renderer.debug_loaded_visual_count() >= 8, "quarter module visual renderer loaded current module set")
	_expect(game.quarter_renderer.debug_visual_variant_key("entrance") == "ne_se", "entrance visual variant key reflects open sockets")
	...
	_expect(game.quarter_renderer.debug_visual_variant_key("slot_01") == "nw_se", "build slot visual variant key reflects open sockets")
