...
func _check_castle_stage_expansions() -> void:
	...
	_expect(stage_01_active_count < game.graph.debug_active_rect().size.x * game.graph.debug_active_rect().size.y, "stage 01 hides locked max-grid rock cells")
	...
	var stage_02_active_count: int = game.graph.debug_active_cells().size()
	_expect(game.graph.debug_unlocked_room_grid_ids().size() == 7, "stage 02 unlocks seven room-grid cells")
	_expect(stage_02_active_count > stage_01_active_count, "stage 02 visibly expands the active grid")
	_expect(game.graph.debug_tile_visual_scale() < stage_01_scale, "stage 02 camera fits the newly expanded east grid")
	...
	var stage_03_active_count: int = game.graph.debug_active_cells().size()
	_expect(game.graph.debug_unlocked_room_grid_ids().size() == 9, "stage 03 unlocks nine room-grid cells")
	_expect(stage_03_active_count > stage_02_active_count, "stage 03 visibly expands the active grid")
	...
	_expect(game.graph.debug_unlocked_room_grid_ids().size() == 11, "stage 04 unlocks eleven room-grid cells")
	_expect(game.graph.debug_active_cells().size() > stage_03_active_count, "stage 04 visibly expands the active grid")
