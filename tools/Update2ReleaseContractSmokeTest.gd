...
func _test_next_cycle_save_and_continue() -> void:
	...
	game.selected_monster_id = "spore_healer"
	var legacy_candidate: Dictionary = game._campaign_legacy_candidate()
	_expect(str(legacy_candidate.get("species_id", "")) == "slime", "계약 몬스터 선택 시 다음 회차에 존재하는 코어 계승 후보로 보정")
