...
func _test_catalog_profile_ui_and_save() -> void:
	...
	root._reset_run_metrics()
	root.campaign_profile = {
		"active_doctrine_id": "",
		"active_decree_id": "",
	...
	root.selected_contract_ids.assign(["contract_test_a", "contract_test_b"])
	root.reserve_instance_ids.assign(["contract_test_reserve"])
	root.monster_roster = {
		"contract_test_a": {"bond": 80},
		"contract_test_b": {"bond": 80}
	}
	root.combat_scene.update2_counter_activations.clear()
	root.leon_adaptation = {}
	root.last_security_grade = ""
	root.completed_raids.clear()
	GameState.infamy = 0
	root._record_final_run_metrics()
	var runtime_metrics: Dictionary = root.run_metrics_tracker.snapshot()
	_expect(int(runtime_metrics.get("update2.cycle_index", 0)) == 2 and int(runtime_metrics.get("update2.contract_selected_count", 0)) == 2, "실제 회차·계약 상태를 Update 2 엔딩 지표로 기록")
	_expect(is_equal_approx(float(runtime_metrics.get("update2.contract_bond_average", 0.0)), 80.0) and int(runtime_metrics.get("update2.reserve_count", 0)) == 1, "실제 계약 유대·예비대 상태를 엔딩 지표로 기록")
	_expect(root._resolve_campaign_ending() == "contract_monster_alliance", "실제 GameRoot 상태에서 E05 계약 마물 연합 엔딩 도달")
	root._onboarding_reset_game()
	root._debug_skip_onboarding()
	GameState.player_name = "도감 검증 마왕"
