...
func _run() -> void:
	...
	for stage_value in stages:
		...
		_expect(snapshot.get("lane_routes", {}).is_empty(), "%s legacy layout does not invent lane routes" % stage["id"])
		_expect(snapshot.get("route_starts", {}).is_empty(), "%s legacy layout keeps the single route_start contract" % stage["id"])
		_expect(snapshot.get("defense_zones", []).is_empty(), "%s legacy layout does not invent defense zones" % stage["id"])
		_expect(snapshot.get("facility_slot_contracts", []).is_empty(), "%s legacy facility slots remain unchanged" % stage["id"])
