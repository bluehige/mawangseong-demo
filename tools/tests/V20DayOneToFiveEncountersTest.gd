...
func _test_catalog_contract() -> void:
	...
	for encounter_id_value in DataRegistry.v20_encounters.keys():
		...
		_expect(encounter.get("preview", {}).get("targets", []).size() >= 1 and encounter.get("preview", {}).get("route_ids", []).size() >= 1, "%s 목표·예상 경로 preview" % encounter_id_value)
...
func _test_schedules_and_routes() -> void:
	...
	for day in range(1, 6):
		...
		var all_routed := schedule.all(func(row): return not row.get("route_nodes", []).is_empty() and str(row.get("route_signature", "")) != "")
		_expect(all_routed, "DAY %d 모든 spawn 목표·weighted route 확정" % day)
	...
	_expect(str(north_engineer.get("route_signature", "")) != str(south_engineer.get("route_signature", "")), "공병 목표 시설 배치가 실제 접근 경로 변경")
...
func _test_wave_manager_adapter() -> void:
	for day in range(1, 6):
		...
		_expect(not entries.is_empty() and entries.all(func(entry): return int(entry.get("count", 0)) == 1 and entry.has("v20_phase_id") and entry.has("v20_route_nodes")), "DAY %d 기존 WaveManager 호환 adapter" % day)
