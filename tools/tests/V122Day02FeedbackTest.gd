...
func _test_contextual_management_ui() -> void:
	...
	var workspace_source := FileAccess.get_file_as_string("res://scripts/ui/ManagementWorkspaceUI.gd")
	var build_body := _function_body(source, "build_management_ui")
	_expect(build_body.contains("workspace_ui.build") and workspace_source.contains("_build_monster_roster_dock"), "관리 화면은 확장 가능한 몬스터 로스터 도크를 조립한다")
	...
	_expect(workspace_source.contains("effect_summary") and workspace_source.contains("recommend_summary"), "시설 목록 tooltip은 실제 효과와 추천 위치를 사용한다")
