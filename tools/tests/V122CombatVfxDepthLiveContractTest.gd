...
func _run() -> void:
	...
	var front_depth := int(renderer.vfx_front_depth())
	var aerial_depth := mini(unit_depth + 12, front_depth - 1)
	var unit_bounds: Vector2i = renderer.unit_depth_slot_bounds()
	_expect(unit_depth >= unit_bounds.x and unit_depth <= unit_bounds.y, "유닛이 활성 지도의 유한 깊이 범위 안에 있음")
	if renderer._prepared_maze():
		_expect(unit_bounds == Vector2i(51,94), "준비된 미궁 몸체는 벽 뒤 픽셀을 제외해 합성")
		_expect(unit_depth > renderer.front_wall_depth(), "마스킹한 몸체는 벽 렌더 이후에 합성")
	else:
		_expect(unit_bounds == Vector2i(1,44), "기존 지도는 전면 벽 아래의 슬롯을 유지")
		_expect(unit_bounds.x < object_front_layer.z_index and object_front_layer.z_index < unit_bounds.y, "기존 지도 소품과 유닛 깊이 유지")
	...
	_expect(top_depth == unit_bounds.x, "맵 최상단 유닛 슬롯")
	_expect(bottom_depth == unit_bounds.y, "맵 최하단 유닛 슬롯")
	_expect(aerial_depth >= unit_depth and aerial_depth < front_depth, "공중 효과가 새 몸체 층 아래로 떨어지지 않음")
