...
func _test_enemy_data() -> void:
	...
	_expect(not bool(spore.get("placeholder_art", true)) and not bool(root.get("placeholder_art", true)) and not DataRegistry.enemies.has("boss_mirella"), "일반 적 최종 그래픽·미렐라 보스 조기 등장 금지")
	_expect(int(spore.get("frame_count", 0)) == 16 and int(root.get("frame_count", 0)) == 16 and FileAccess.file_exists(str(spore.get("sprite", ""))) and FileAccess.file_exists(str(root.get("sprite", ""))), "포자 인형·뿌리 정원사 최종 16프레임 시트")
