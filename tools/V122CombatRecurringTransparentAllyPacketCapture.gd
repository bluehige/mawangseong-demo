...
const OUTPUT_DIR := "res://tmp/v122_release_polish/v2_p4"
const OUTPUT_IMAGE := "v2_p4_transparent_contract_ally_contact_sheet_1280x720.png"
const OUTPUT_MANIFEST := "v2_p4_transparent_contract_ally_capture_manifest.json"
const GRID_COLUMNS := 3
const CELL_SIZE := Vector2(378.0, 276.0)
const GRID_ORIGIN := Vector2(44.0, 98.0)

const PACKETS := [
	{"id": "spore_healer", "sprite": "res://assets/sprites/monsters/monster_slime_idle_down_00.png", "median": 118.5},
	{"id": "stone_sentinel", "sprite": "res://assets/sprites/monsters/monster_slime_idle_down_00.png", "median": 118.5},
	{"id": "war_drummer", "sprite": "res://assets/sprites/monsters/monster_goblin_idle_down_00.png", "median": 150.0},
	{"id": "mimic_porter", "sprite": "res://assets/sprites/monsters/monster_goblin_idle_down_00.png", "median": 150.0}
]
...
func _run() -> void:
	...
	await _save_capture()
	_write_manifest()
	print("V122_COMBAT_TRANSPARENT_CONTRACT_ALLY_CAPTURE: %s records=%d image=%s" % [capture_status, capture_records.size(), capture_image_size])
...
func _build_contact_sheet() -> void:
	for index in range(PACKETS.size()):
		...
		var stats: Dictionary = DataRegistry.monster(unit_id).duplicate(true)
		if stats.is_empty() or not FileAccess.file_exists(sprite_path):
			capture_status = "INVALID_INPUT"
			push_error("Missing contract ally stats or asset: %s" % unit_id)
			continue
		stats["sprite"] = sprite_path
		stats["display_name"] = unit_id
		stats["role"] = "visual_audit"
		var profile: Dictionary = DataRegistry.combat_visual_profile_for_unit(unit_id, sprite_path)
		if profile.is_empty():
			capture_status = "INVALID_INPUT"
			push_error("Missing contract ally visual profile: %s" % unit_id)
			continue

		var unit := UnitScript.new()
		unit.name = "V122TransparentContractAlly_%s" % unit_id
		add_child(unit)
		unit.setup(unit_id, stats, "monster", "visual_audit")
...
func _draw() -> void:
	...
	draw_string(UI_FONT, Vector2(34, 34), "v1.2.2 V2-P4 투명 전투 원본 계약 아군", HORIZONTAL_ALIGNMENT_LEFT, 760, 22, Color("#f2e6ca"))
	draw_string(UI_FONT, Vector2(34, 61), "계약 아군 4개 · 공유 기존 원본은 크기/알파 패킷에서만 검증 · 코볼트 지원 원본 제외", HORIZONTAL_ALIGNMENT_LEFT, 1130, 13, Color("#abb4c2"))
	...
	for index in range(capture_records.size()):
		...
		var outline := Color("#b79048") if state == "NEEDS_NORMALIZATION" else Color("#4e9b63")
		draw_rect(cell_rect, Color("#282415") if state == "NEEDS_NORMALIZATION" else Color("#16251d"), true)
...
func _save_capture() -> void:
	...
	if texture == null:
		...
		push_error("V2-P4 viewport texture is unavailable; run without --headless")
		return
	var image := texture.get_image()
	if image == null or image.is_empty():
		capture_status = "CAPTURE_BLOCKED"
		push_error("V2-P4 capture viewport is empty; run without --headless")
		return
	capture_image_size = image.get_size()
	if capture_image_size != REFERENCE_VIEWPORT:
		capture_status = "CAPTURE_SIZE_MISMATCH"
		push_error("V2-P4 capture size mismatch: %s" % capture_image_size)
		return
	...
	if error != OK:
		...
		push_error("V2-P4 capture save failed: %s" % error)
		return
	capture_status = "PASS"
...
func _write_manifest() -> void:
	...
	if file == null:
		push_error("V2-P4 manifest open failed: %s" % manifest_path)
		return
