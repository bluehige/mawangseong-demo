...
1. 탑뷰 마왕성 안에서 몬스터와 적이 실시간으로 움직이며 싸우는가.
...
```text
cute horror fantasy top-down 2D game asset, charming spooky cave demon castle style, dark stone, purple magical glow, adorable SD proportions, readable silhouette, clean hand-painted indie game style, transparent background, no text, no logo, no watermark
```

### 14.4 슬라임 프롬프트

```text
Create a top-down 2D game character sprite for a cute blue slime monster, tank role, adorable round body, glossy jelly surface, tiny determined face, small shield-like aura, cute horror fantasy demon castle style, 128x128 canvas, transparent background, centered, readable silhouette, no text, no watermark.
...
```text
Create a top-down 2D game character sprite for a cute green goblin warrior, melee damage dealer, small dagger, mischievous expression, SD proportions, cute horror fantasy demon castle style, 128x128 canvas, transparent background, centered, readable silhouette, no text, no watermark.
...
```text
Create a top-down 2D game character sprite for a cute red imp magic caster, small horns, tiny bat wings, yellow eyes, holding a little fireball, adorable but spooky, SD proportions, cute horror fantasy demon castle style, 128x128 canvas, transparent background, centered, readable silhouette, no text, no watermark.
...
```text
Create a top-down 2D game enemy sprite for a cute human explorer invading a demon castle, small hat, torch, backpack, nervous expression, readable silhouette, SD proportions, cute horror fantasy style, 128x128 canvas, transparent background, centered, no text, no watermark.
...
```text
Create a seamless 64x64 top-down 2D cave floor tile for a cute horror fantasy demon castle, dark stone floor, subtle cracks, purple ambient light, hand-painted indie game style, no text, no objects, tileable, no watermark.
```

```text
Create a 64x64 top-down 2D cave wall tile for a cute horror fantasy demon castle, dark stone wall, purple magical rim light, readable edges, hand-painted indie game style, tileable, no text, no watermark.
```
...
- 탑뷰 맵과 UI가 예시 이미지와 같은 큐트 호러 톤을 가진다.
...
```text
Godot 4.5와 GDScript로 2D 탑뷰 데모 게임을 구현해줘. 게임명은 「마왕님, 마왕성은 누가 지켜요?」이고, 장르는 큐트 호러 판타지 마왕성 방어 시뮬레이션이다.
...
- [x] 3일차 견습 용사를 잡으면 데모 클리어가 뜬다.
- [x] 결산 화면이 표시된다.

2026-07-01 기준 `tools/DemoSmokeTest.tscn` 자동 스모크 테스트 통과.
추가로 `tools/ManualVerificationCapture.tscn` 렌더링 캡처 확인, `GameRoot.gd` 관리/전투/HUD 컨트롤러 분리, `AnimatedSprite2D` + `SpriteFrames` 애니메이션 구조 적용 완료.
