...
이 프로젝트에서 `gpt-image-2` 또는 `GPT Image 2`로 이미지 리소스를 만든다는 말은 Codex 대화 안의 **내장 이미지 생성 도구**를 사용한다는 뜻이다.

금지:

1. API/CLI fallback을 기본 경로로 해석하지 않는다.
2. `OPENAI_API_KEY` 확인을 요구하지 않는다.
3. 별도 이미지 생성 스크립트로 우회하지 않는다.
4. 사용자가 이미지 리소스를 요청했는데 절차 생성 도형, SVG, 코드 드로잉, placeholder PNG로 대체하지 않는다.

허용:

1. 내장 이미지 생성 결과를 프로젝트 폴더로 복사한다.
2. 생성된 이미지를 크롭, 알파 제거, 리사이즈, 슬라이스, 색 보정한다.
3. Godot import와 manifest 연결을 코드로 처리한다.
