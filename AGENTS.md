...
- 화면에는 `1.2`처럼 표시하고 프로젝트·manifest·태그에는 SemVer `1.2.0`을 쓴다. 정확한 출시본은 이동하지 않는 태그(`v1.0.0`, `v1.1.0`, `v1.2.0`, `v2.0.0`)로 보존한다.
- 현재 수정판 통합은 `release/v1.2`, 다음 확장 통합은 `release/v2.0`, `release/v3.0`, `release/v4.0`에서 진행한다.
