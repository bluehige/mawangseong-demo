...
- 화면에는 `1.2`처럼 표시하고 프로젝트·manifest·태그에는 SemVer를 쓴다. 현재 공개 안정판은 `1.2.5`이고 다음 정식 확장판은 `2.0.0`이다. 정확한 출시본은 이동하지 않는 태그(`v1.0.0`, `v1.1.0`, `v1.2.0`~`v1.2.5`, 이후 승인된 `v2.0.0`)로 보존한다.
- 현재 유지보수선은 `1.2.5`다. 개별 구현은 `codex/v125-<topic>`, 검수는 `test/v125-<topic>` 형식을 사용한다.
