...
- 화면에는 `1.2`처럼 표시하고 프로젝트·manifest·태그에는 SemVer를 쓴다. 현재 공개 안정판은 `1.2.3`이고 다음 정식 확장판은 `2.0.0`이다. 정확한 출시본은 이동하지 않는 태그(`v1.0.0`, `v1.1.0`, `v1.2.0`, `v1.2.1`, `v1.2.2`, `v1.2.3`, 이후 승인된 `v2.0.0`)로 보존한다.
- 현재 선별 통합은 `release/v1.2.3`에서 진행한다. 개별 구현은 `codex/v123-<topic>`, 검수는 `test/v123-<topic>` 형식을 사용한다.
...
- `1.2.3` 출시 완료 뒤 실제 확장 통합은 최신 `main`에서 새로 시작하며 `release/v3.0`, `release/v4.0`도 같은 원칙을 따른다.
