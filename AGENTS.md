...
- 화면에는 `1.2`처럼 표시하고 프로젝트·manifest·태그에는 SemVer를 쓴다. 현재 공개 안정판은 `1.2.2`이고 다음 패치 후보는 `1.2.3`이다. 정확한 출시본은 이동하지 않는 태그(`v1.0.0`, `v1.1.0`, `v1.2.0`, `v1.2.1`, `v1.2.2`, 이후 승인된 `v1.2.3`, `v2.0.0`)로 보존한다.
