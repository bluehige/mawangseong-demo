...
| 3 | B | 명령 없음 |
| 4 | A | 궁수 world position과 Z2 감시 초소 anchor의 거리가 처음 420px 이하가 된 physics frame 뒤 1초 안에 감시 초소 발동 |
| 4 | B | 궁수 spawn 뒤 1초 안에 궁수에게 `적 집중` |
| 5 | A | 용사 대시 예고 시작 뒤 1초 안에 Z4로 `비상 후퇴`; 부상한 생존 몬스터 한 마리 이상이 Z4 `combat_bounds` 안에 들어온 다음 physics frame에 회복 둥지 발동 |
| 5 | B | 명령 없음 |
...
| 3 | Z1 바리케이드 + 슬라임·고블린, Z4 임프, 명령 없음 | `primary_success == false`, 시설 최대 연속 비활성 2초 이상, 무력화 interval 중 다른 시설 effect 0건, `failed_objective == keep_one_facility_active` |
| 4 | 시설 없음, Z1 슬라임·고블린, Z4 임프, 궁수 집중 없음 | `primary_success == false && rear_pressure_seconds >= 6.0 && failed_objective == break_rear_pressure` |
| 5 | 시설 없음, Z1 슬라임·고블린, Z2 임프, 후퇴·집결 없음 | `primary_success == false`이고 (`fallback_breaches >= 1` 또는 `second_phase_leaks >= 1` 또는 `gold_stolen >= 1`) |
...
- DAY 2 도난, DAY 3 무력화 중 대체 시설 effect 부재, DAY 4 후열 압박 6.0초 이상, DAY 5 후퇴 돌파·2차 누수·도난은 상위 계약 8절의 `failed_objective`와 `RESULT.win=false`에 일치
...
PR 4 첫 seed 20개 후보 수치는 상위 계약 9.1 표에만 기록한다. 이 20개는 seed당 1회라서 위 70전의 분자나 분모에 넣지 않으며, `PHYSICAL_COMBAT_PASS`, 수동 PASS, 초회 사용자 PASS 또는 재미·밸런스 PASS로 보고하지 않는다.
...
- 관찰자는 고정 SHA kit의 `Start-Test.cmd`를 실행하고 참가자 ID `FU01`~`FU10` 중 하나만 입력한다. 같은 ID의 session 폴더가 이미 있으면 launcher는 덮어쓰거나 삭제하지 않고 실행을 거부한다.
- launcher는 자기 process에서만 `APPDATA=<session>/AppData/Roaming`, `LOCALAPPDATA=<session>/AppData/Local`을 사용한다. 각 session은 다른 `user://v20/`과 `user://v20_acceptance/`를 가지며 launcher 종료 뒤 원래 환경 변수를 복원한다.
- 게임에는 고정 acceptance 인자와 함께 `--v20-participant=<FU ID>`, `--v20-evidence-kind=human_first_user`, `--v20-source-sha=<40자 full SHA>`를 전달한다. participant 인자가 없는 자동·물리 실행에서는 참가자 기록기를 켜지 않는다.
- 참가자에게는 실행 방법과 종료 조건만 말한다. 배치 후보, 시설 효과, 적 대응, DAY 순서와 고정 질문의 정답을 말하지 않는다.
- 게임 process가 끝난 뒤 launcher가 6.4의 다섯 질문을 그대로 묻는다. 자유 답변에는 이름·연락처를 적지 말라고 먼저 표시하고, 5점 문항은 1~5 정수만 받는다.
- 관찰자는 도움 문장과 시각, 외부 장치 실패, crash, soft lock과 진행 차단을 별도 항목에 기록한다. 게임 crash·hard lock·save 오류와 결과 미도달을 외부 실패로 바꾸지 않는다.
- 정상 종료, 30분 종료와 비정상 종료 모두 `<FU ID>_result.zip`을 만든다. ZIP 생성 실패도 session 실패로 남기며 원본 session 폴더를 삭제하지 않는다.

결과 ZIP은 아래 파일을 포함한다.

| 파일 | 필수 내용 |
|---|---|
| `manifest.json` | participant ID, evidence kind, source·EXE·PCK hash, 시작·종료 시각, process exit code·종료 사유, 내부 파일 hash, 기존 save sentinel 전후 hash |
| `session_metrics.json` | title 노출부터 첫 배치·방어 시작까지 초, 상태별 체류, invalid 시도, 20초 이상 입력 공백, DAY별 시도와 terminal 사유 |
| `gameplay_events.json` | 상태 전이, 성공·실패 배치 시도, 결과 뒤 재도전·다음 DAY 행동 |
| `combat_events.json` | DAY·attempt별 raw placement와 `metrics.v20_evidence` 전체 |
| `feedback.json` | 6.4 고정 질문 원문 답변과 1~5점 |
| `observer_notes.json` | 개입 횟수·정확한 문장·시각, 외부 실패, crash·soft lock·진행 차단 |
| `runtime.log` | Godot `--log-file` 출력과 launcher process exit 정보 |

이 ZIP을 만들었다는 사실은 `FIRST_USER_PASS`가 아니다. `evidence_kind=synthetic_preflight` 또는 `participant_id=AGENT-*`인 ZIP은 kit 사전 점검에만 사용하고 FU01~FU10 분자·분모에서 제외한다.
