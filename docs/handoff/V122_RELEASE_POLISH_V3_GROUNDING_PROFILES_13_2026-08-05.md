# v1.2.2 출시 마무리 핸드오프 — V3 dusk_courier 시트 앵커 13
...
- QA 문서: `docs/qa/V122_V3_GROUNDING_PROFILES_13_2026-08-05.md`

## 이번 패킷에서 한 일

`dusk_courier` 투명 4×4 런타임 시트의 192×192 셀을 직접 측정했다. `Unit.gd` 시트 매핑에 따라 idle 셀 `(0,0)`의 발 앵커는 `0.984375`, down 셀 `(2,0)`의 발 앵커는 `0.953125`이며 alpha bbox는 각각 `[37,43,163,189]`, `[16,79,171,183]`이다.

기존 `DataRegistry`의 `normal_flying` 런타임 병합 경로와 `Unit.gd`의 투명 시트 AtlasTexture 경로를 그대로 사용해 `motion_entry`와 `VisualBody` 위치가 이 값을 소비하는지 확인했다. 투명 시트의 chroma material 미적용과 기존 render scale은 유지했다.
...
1. 다음 단일 패킷은 `V3-GROUNDING-PROFILES-14`로 `bronze_automaton` 한 캐릭터의 4×4 런타임 시트 idle/down 셀 앵커를 같은 경로로 처리한다.
2. `dusk_courier`를 포함한 Update 4 원본 `NEEDS_NORMALIZATION` 상태의 최종 승격, 나머지 계약 캐릭터의 개별 프로필, 대표 동작, V4 전면 벽 가림은 이후 별도 패킷으로 남긴다.
