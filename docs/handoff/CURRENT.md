...
- **2026-09-22 최우선: [1.2.9 Steam 반려 수정](V129_STEAM_REVIEW_FIX_2026-09-22.md).** Build25423797은 실제 입력 불가로 반려됐다. 전체 화면 튜토리얼 Panel이 마우스 통과 설정을 무시하고 지도를 차단한 원인을 재현·수정했다. 한/영 실제 클릭·드래그·전투 진입과 관련 기존 검사 통과. 1.2.9 소스 검증 완료, 태그/Steam 업로드·재심사 제출은 아래 후속 기록이 생기기 전까지 미완료다. 과거 부팅 확인을 실제 첫 플레이 입력 검증으로 해석하지 않는다.
...
0. [1.2.9 반려 수정 기록](V129_STEAM_REVIEW_FIX_2026-09-22.md)의 남은 태그·패키지·Steam default 교체·빌드 재심사를 먼저 마친다. Build25423797 심사는 더 이상 대기 상태가 아닌 반려 상태다.
