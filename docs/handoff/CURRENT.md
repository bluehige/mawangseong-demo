...
- 작업명: 앞벽이 몸체를 덮지 않던 결함을 실제 전경 벽으로 수정
...
- Reviewed implementation SHA: c2afafdbbf259507f2ec0e9ff22b7d585202e771
- 최신 핸드오프: [UIUX_V2_REAL_FOREGROUND_WALL_2026-09-13.md](UIUX_V2_REAL_FOREGROUND_WALL_2026-09-13.md)
- 사용자 지적으로 이전 몸체 알파 혼합이 실제 전경 가림을 표현하지 못했음을 확인했다. 이전 시각 완료 판정을 이번 수정으로 대체한다.
- 기존 벽 면·재질 색상 캐시와 실제 전경 z108을 추가했다. 몸체 위에 벽면42%·윗단72%로 그리고 주변 바닥도 비치게 한다. 관리 몸체107·전투 몸체51~94·이름/UI110 순서다.
- 실제 발 깊이·몸체 주변 영역으로 앞뒤를 판단한다. 캐릭터 제거 시 투명 영역을 비우고 지도 이탈 시 전경 벽을 숨긴다. 기존 높이112·기둥120·돌바닥을 유지한다.
- Windows1080p/720p × 글자90/100/115%: 실제 전경 벽973 assertions·31장, 미궁31,170 assertions·100장, 건설344 checks·36장 PASS. 이전 잘못된 혼합 검사가 아니라 실제 전경 벽 켜기/끄기/불투명 대조군으로 검증했다.
- 저장소 정책 PASS: 700개 최종 경로 / 66커밋, 1bfc6eca981034dd53a442e072d1422a87913de0 기준. 이후 핸드오프 결과 기록만 추가.
- 비교: tmp/uiux_real_wall_overlay_20260913/index.html. 픽셀 근거 final/depth_results.json, 미궁 근거 matrix/results.json.
- 돌바닥 SHA256 8f50dfb9f466bd0aeb054e249efbde915b28da966f350f99069b9302eb847d7c 유지. 신규 그림/오디오/자산 교체 없음.
- 시설·아치 자체의 정밀 깊이 판정은 별도 보완 영역이다. 추가 벽 이미지 캐시와128개 표시 몸체 한도가 있으며 대규모 군중·장시간 GPU·Web/모바일은 미검증.
- 게임 규칙·경로·비용·저장 변경 없음. 주 에이전트 단독 수행. 전체 캠페인·사람·8인 검수 미실행.
- 새 버전·공개 배포·main·태그 변경 없음. 제품 권위는 main:CURRENT다.
