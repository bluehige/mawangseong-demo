...
- 기본 작업은 변경 범위에 직접 관련된 테스트만 실행한다.
- 전체 회귀, 전체 플레이와 별도 검수 에이전트는 사용자가 현재 작업에서 명시적으로 요청한 경우에만 실행한다.
- 2026-07-13 저장소 문서화 중 시작한 Full 검증은 사용자 지시에 따라 마지막 액션 시각 검사 단계에서 중단했다. 완료 근거로 사용하지 않는다.
...
4. 사용자가 v0.3 정식 출시 검증을 명시적으로 요청하면 전체 자동 검증, 저장 호환성, Web 내보내기와 실제 브라우저 검수를 실행한다.
5. 사용자가 별도 검수 에이전트를 요청한 경우에만 해당 지적을 수정하고 요청 범위에서 재검수한다.
