...
- 작업 단계: 보물고·회복 둥지·감시초소 3종 × 4방향 및 지도 건설 표식을 실제 게임에 연결하고 표적 검사 완료. 전체 미술 완료 아님.
- 작업 브랜치: codex/v126-uiux-u0-u3 (로컬, 미푸시)
- 기준 main/origin/main/원격 main: 69a75970b1f8c030aa3a6956e5ca0f5bf15b2112 (2026-09-13 원격 조회)
- Reviewed implementation SHA: 9a46ad8d01722cca6f58da648abbe81c1456b223
- 최신 핸드오프: [UIUX_V2_FACILITIES_2026-09-13.md](UIUX_V2_FACILITIES_2026-09-13.md)
- 이전 판석 UI·병영·대표 캐릭터: [UIUX_V2_ART_DIRECTION_2026-09-12.md](UIUX_V2_ART_DIRECTION_2026-09-12.md)
...
- 새 시설 12장은 GPT 내부 네이티브 RGBA, 원본/런타임 바이트 동일, 축소 저장 없음. 기본 방향 12경로와 성 2단계 NW 별도 선택 2경로 연결.
- 성 1·3·4 NW 전용 그림, 기존 병영·수호핵·대표 캐릭터 8종 및 게임 규칙/저장 구조 보존.
- 최종 시설 직접 검사 1,365개/배치 54사례, 지도 표식 건설 안전 검사 343개, 기존 시각 계약 2종 PASS.
- Windows 1920×1080 / 1280×720 × 글자 90/100/115%, 전후 각 189장: tmp/uiux_facilities_20260913/index.html.
- SW는 현재 기본 지도에 변경 가능한 슬롯이 없어 자산 계약 확인만 수행. 실제 SW 배치 PASS로 기록하지 않음.
- 새 런타임 PNG 17.39 MiB. 단계 순회 종료 시 텍스처 메모리 약 531.43 MiB 관측, 저사양 성능 판정 아님.
- 나머지 시설·단계 원본·후반 캐릭터·큰 초상·보조 메뉴 구조와 전체 미술 통일은 남아 있다.
- 전체 캠페인·사람 사용성·다른 플랫폼·8인 검수·출시는 미실시. 새 버전 미확정, 공개 1.2.6과 기존 태그 불변.
...
1. 사용자의 최신 순서: 지정 참고 기반 UI 그래픽 → 깨진 건물 외형 → 캐릭터/적 입체감. 완료한 병영·수호핵·시설 3종을 유지하고 입구·왕좌·빈 기반 및 남은 성 단계 원본을 실제 게임 크기로 개선한다.
2. 후반 적·동료·진화/왕관·큰 초상을 같은 입체 재질로 확장한다. 이미 완성된 수호핵 6장, 병영 4방향, 보물고/회복 둥지/감시초소 12장, 대표 8종을 미완료로 다시 생성하지 않는다.
