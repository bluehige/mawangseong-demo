...
- **2026-09-22 최우선: [1.2.9 Steam 반려 수정](V129_STEAM_REVIEW_FIX_2026-09-22.md).** Build25423797은 실제 입력 불가로 반려됐다. 전체 화면 튜토리얼 Panel이 마우스 통과 설정을 무시하고 지도를 차단한 원인을 재현·수정했다. 한/영 실제 클릭·드래그·전투 진입과 관련 기존 검사 통과. 1.2.9 소스·로컬Windows후보ZIP·한영EXE부팅·컴파일PCK첫플레이 검증 완료, PR104 정책PASS. 자동 승인 검토가 이전1.2.8 지정과 새1.2.9의 차이를 사유로 main병합/태그를 차단하여 사용자에게 버전 승인을 요청했다. 답변 전까지 태그/Steam업로드·재심사 제출은 미완료다. 과거 부팅 확인을 실제 첫 플레이 입력 검증으로 해석하지 않는다.
...
0. [1.2.9 반려 수정 기록](V129_STEAM_REVIEW_FIX_2026-09-22.md)8절의 버전 승인 응답을 먼저 반영한다. 로컬 후보는 준비됐지만 main병합/태그/Steam업로드를 완료한 것으로 간주하지 않는다. 승인된 버전으로 최종 태그·패키지·Steam default 교체·빌드 재심사를 마친다. Build25423797 심사는 더 이상 대기 상태가 아닌 반려 상태다.


1. [한국 출시 후속 기록](V128_KOREA_RELEASE_2026-09-21.md)과 [상점 마무리 세션](V128_STEAM_STORE_POLISH_2026-09-21.md)을 기준으로 이어간다. 게임위 원본 확인은 완료됐으므로 이전 유통통보 미확인 기록을 현재 상태로 사용하지 않는다. 한국 등급 안내는 상점 본문에 저장됐고 사용자가 별도 KGRB 12 등록까지 완료한 것을 확인했다. Cloud 정확한 경로6개도 개발자 테스트 모드로 게시했다. 지원 문의는 사용자 지시대로 초안만 보관한다. 기본1.2.8 활성화·전체 영어화·한영 소개·대표/라이브러리 이미지 업로드·갤러리 정리는 완료됐으므로 반복하지 않는다.
