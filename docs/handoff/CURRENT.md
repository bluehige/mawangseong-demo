...
- 새 11장 모두 GPT 내부 네이티브 RGBA, 원본/런타임 바이트 동일, 축소 저장 없음. 단계별 병영·정예 병영·보물고·회복·감시초소의 기존 선택을 교체.
- 휠 확대·중간 버튼 이동·+/−·지도 전체를 연결. 건설/몬스터 배치 중 잠금, UI 위 입력 차단, ESC·포커스 상실·화면 전환 처리.
- 지도 이름표는 기존 글자 배율을 반영하며 확대에 따라 작아지지 않는다. 모인 몬스터 이름표는 겹치지 않게 벌리고 연결선 표시.
- Windows 1920×1080 / 1280×720 × 글자 90/100/115%. 시설 66배치 1,887개, 지도 24조건 1,062개, 기존 건설 안전 343개, 관련 시각 계약 3종 PASS.
- 같은 장면 전후 238/238장, 새 지도 조작 96장과 안전 검사 36장: tmp/uiux_stage_art_20260913/index.html.
- 신규 런타임 PNG 18.99 MiB. 단계 순회 종료 텍스처 메모리 약 607.26 MiB 관측, 저사양 성능 판정 아님.
- 남은 후반 캐릭터·큰 초상·보조 메뉴 구조와 전체 미술 조화는 후속 작업.
...
1. 사용자의 최신 우선순위를 유지하며 후반 적·동료·진화/왕관·큰 초상을 실제 연결 경로에서 조사하고 입체 재질로 확장한다. 기존 프레임·발 기준점·AI/수치는 보존한다.
2. 완료한 단계 전용 시설 11종, 성문·왕좌·빈 기반 15종, 수호핵 6장, 병영 4방향, 기본 시설 12장과 대표 캐릭터 8종을 재생성하지 않는다. 배경·조명과의 조화 및 다수 몬스터 이름표 밀도는 추가 시각 확인한다.
