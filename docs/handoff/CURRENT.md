...
- PR #10의 `repository-policy`가 PASS했고 merge commit `ba661015`로 `main`에 통합했다.
- 정식 SemVer 태그 `v0.3.0`을 같은 merge commit에 생성·푸시했다.
...
1. 최신 `main`에서 v0.4 전용 브랜치를 만들고 Phase 0→Phase 36 순서로 개발한다.
2. v0.4 완료 후 v0.5, 그다음 v0.6을 같은 방식으로 순차 진행한다.
3. 각 버전의 지정 그래픽 Phase에서만 GPT 내부 이미지 생성과 source/runtime 분리를 수행한다.
