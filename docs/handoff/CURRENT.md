...
1. [한국 출시 후속 기록](V128_KOREA_RELEASE_2026-09-21.md)과 [상점 마무리 세션](V128_STEAM_STORE_POLISH_2026-09-21.md)을 기준으로 이어간다. 게임위 원본 확인은 완료됐으므로 이전 유통통보 미확인 기록을 현재 상태로 사용하지 않는다. 한국 등급 안내는 상점 본문에 저장됐고, 별도 KGRB 배지는 미등록이다. 지원 문의는 사용자 지시대로 초안만 보관한다. 기본1.2.8 활성화·전체 영어화·한영 소개·대표/라이브러리 이미지 업로드·갤러리 정리는 완료됐으므로 반복하지 않는다.
