...
- 제품 2.0 GPT Pro 복구 기획 요청: `docs/briefs/GPT_PRO_V20_RECOVERY_REQUEST_2026-07-22.md`
- 제품 2.0 GPT Pro 복구 기획 요청 핸드오프: `docs/handoff/V20_GPT_PRO_RECOVERY_BRIEF_2026-07-22.md`
...
- Phase 11T는 고정 경로가 왕좌까지 시각적으로 이어지더라도 실제 적은 `성문 전초 → 가시 회랑 → 중앙 전투실 → 왕좌 전실`을 순서대로 돌파해야만 왕좌로 갈 수 있게 checkpoint 전투를 고정했다. `d1829eb21951ba8d1b4aebdf3103c148c4524c9e`에서 실제 몬스터 초상 카드 3종과 drag preview를 추가하고, 검은 전투 오버레이를 제거했으며, 현재 방어 단계·양측 유닛·시설·네 전술 명령의 대상과 효과를 상시 표시한다. 관련 10개 스위트 462 assertions와 runtime probe, 1280×720 Godot 관리·전투 렌더를 통과했고 소스 PR #63과 `/v20-p11t/` 공개 배포까지 완료했다.
- 후속 실제 플레이와 코드 계보 감사에서 현재 Phase 11T는 제품 전투 코어 기준 `NO-GO`로 판정했다. 기존 1.2 AI가 삭제된 것이 아니라 v20의 수동 anchor·fixed route·checkpoint 조기 반환과 전술 명령 우선순위 충돌이 기존 행동을 우회하고, 준비 보드와 runtime 전투 방이 서로 다른 모델이며, 서비스·합성 데이터 중심 테스트가 이를 검출하지 못했다.
- 2.0의 다음 목적은 기능 확장이 아니라 `UI 최적화·단순화`, `몬스터 성장과 시설·몬스터 배치 최적화`, `목적 있는 적 AI와 재미있는 전투 밸런스`를 DAY 1~5에서 먼저 증명하는 것이다. 이 세 축이 수용 기준을 통과하기 전에는 Phase 12와 기존 스토리·전체 전투 시스템 이식을 시작하지 않는다.
- 저장소 근거, 보존·폐기 후보, 필수 설계 질문, 실제 물리 전투 테스트와 최초 구현 PR 산출물을 강제하는 GPT Pro 요청서를 `docs/briefs/GPT_PRO_V20_RECOVERY_REQUEST_2026-07-22.md`와 [Draft PR #65](https://github.com/bluehige/mawangseong-demo/pull/65)에 고정했다. GPT Pro 답변을 받은 뒤 사용자 승인 가능한 새 마스터 명세와 단계별 복구 계획으로 변환한다.
...
1. GPT Pro가 `docs/briefs/GPT_PRO_V20_RECOVERY_REQUEST_2026-07-22.md`와 연결된 저장소·공개 빌드를 확인하고 지정된 형식으로 복구 기획에 답한다.
2. GPT Pro 답변을 저장소 근거와 대조해 `V20_CORE_REBUILD_MASTER_SPEC.md`와 보존·재작업 매트릭스를 현재 고정 동선 요구에 맞게 개정하고, 사용자에게 최종 구현 순서 승인을 받는다.
3. 승인된 첫 복구 PR은 실제 전투와 준비 화면의 단일 맵·상태 소유권 및 기존 AI 우회 제거부터 시작한다. UI 미관·DAY 6~30·스토리 확장은 선행하지 않는다.
4. 실제 물리 프레임 통합 테스트와 내부 수동 플레이가 통과한 DAY 1 후보에서만 6~10명 무설명 블라인드 플레이를 진행한다. 수용 기준 미달이면 다음 페이즈를 차단한다.
5. 세 핵심 축이 모두 Go일 때만 Phase 12 DAY 6~30과 기존 스토리·전체 전투 시스템 이식을 시작한다.
...
9. 사용자 피드백에서 남는 병목이 있으면 해당 플랫폼 프로필만 조정하고 PC·모바일 Pages를 다시 배포한다.
...
- GPT Pro의 저장소 기반 복구 기획 답변과 그 답변에 대한 사용자 승인
- 현재 고정 동선 요구에 맞춘 2.0 마스터 명세·보존 매트릭스 개정
