...
- 작업명: UI·UX V2 — 건물 배치부터 동료·전투·결과·필수 메뉴까지 실제 조작 개편
- WORKSTREAM_ID: UIUX-V2-U0-U5-20260912
- 작업 단계: U0~U3 보완 및 U4/U5 실제 조작 구현·표적 검사 완료. 수호핵 방향별 미술과 일부 보조 메뉴 재배치·사람 사용성은 남음.
- 작업 브랜치: codex/v126-uiux-u0-u3 (로컬, 미푸시)
- 기준 main/origin/main: 69a75970b1f8c030aa3a6956e5ca0f5bf15b2112
- Reviewed implementation SHA: bc52628a4659259daff64949019b159ccbd5381c
- 최신 핸드오프: [UIUX_V2_U4_U5_2026-09-12.md](UIUX_V2_U4_U5_2026-09-12.md)
- 이전 건설·관리 기록: [UIUX_V2_U0_U3_2026-09-12.md](UIUX_V2_U0_U3_2026-09-12.md)
- 미술 미완료/제작 요청: [UIUX_V2_ART_GAPS_2026-09-12.md](UIUX_V2_ART_GAPS_2026-09-12.md)
- Windows 실제 입력 검사 5개 묶음 1,653개 항목과 관련 기존 검사 16개 PASS. 1920×1080 / 1280×720, 글자 90/100/115% 확인.
- 29개 화면 적용 범위와 예외를 최신 핸드오프에 구분했다. 화면 캡처를 모든 선택지/캠페인 진행 성공으로 판정하지 않는다.
- 전체 회귀·사람 사용성·Web/모바일·출시는 미실시. 전체 미술/모든 메뉴 재배치 완료 판정 아님.
...
1. ART_GAPS의 수호핵 단계 3/4 × NE/SE/SW 6개 방향 미술을 제작하고 실제 알파·투영·바닥 접점·카드/고스트/설치 일치를 검사한다. 현재 NW 기존 자산은 실제 게임에서 일관되게 사용한다.
2. 화면 적용표의 보조 메뉴 예외를 기준으로 회차·계약·엔딩/도감·전초기지의 정보 구조와 미술을 추가 개선한다. 필수 기능과 기존 데이터/비용은 보존한다.
3. 사람 사용성을 확인할 때 건물 설치→몬스터 배치→방어 시작 과업을 이전 화면과 비교한다. 현재 5명 시험/90초 완료 수치는 검증되지 않았다.
4. 변경한 기능의 직접 검사만 다시 실행한다. 이번 사용자 지시만으로 전체 DAY 1~30·8인 검수·새 버전·태그·공개 배포를 자동 실행하지 않는다.
