...
- 상점 이미지·스크린샷·라이브러리·트레일러 및 개발사/배급사·지원 연락처는 실제 등록 완료를 확인했다. 사용자에게 파일 업로드를 다시 요구하지 않는다. 최종 재조회에서 가격 USD4.99/KRW5,450·예정일2026-09-28 23:00 KST 등록과 빌드 체크리스트 완료를 확인했다. 상점 필수 미완료는 콘텐츠 설문만이며, 이후 Valve 상점/빌드 심사가 필요하다. 시스템 사양 체크는 통과하지만 입력된 Windows 11 외 최소 성능 측정 근거는 미확정이다.
