...
- 원격 푸시 여부: 미푸시
- 관련 이슈: [#39](https://github.com/bluehige/mawangseong-demo/issues/39)
...
1. 이 핸드오프와 `CURRENT.md`만 문서 커밋하고 이슈 #39용 draft PR을 만든다.
2. PR 필수 상태 검사를 확인하고 merge commit 방식으로 `main`에 통합할 준비를 한다.
3. 물리 한국어 IME와 코드 서명 인증서 적용을 확인한 뒤 `v1.2.1` 출시 여부를 결정한다. 기존 `v1.2.0` 태그는 이동하지 않는다.

## 9. 작업 트리 상태

- 브랜치: `codex/v12-playtest-fixes`
- 미커밋 파일: 이 핸드오프와 `docs/handoff/CURRENT.md`만 남아 있다.
