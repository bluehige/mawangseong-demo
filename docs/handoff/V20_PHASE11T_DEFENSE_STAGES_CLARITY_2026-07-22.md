...
- Web 브랜치 최초 push의 repository-policy run `29900047834`는 GitHub가 신규 브랜치 기준을 `origin/main`으로 잡아 승인 range가 불일치해 실패했다. 빌드·LFS 실패가 아니며, Reviewed SHA 뒤 문서만 보완해 후속 push 검사를 다시 실행한다.
