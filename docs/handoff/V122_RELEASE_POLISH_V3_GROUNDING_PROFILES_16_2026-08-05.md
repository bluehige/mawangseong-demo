# V122 출시 마무리 핸드오프 — V3 spore_doll 프로필 패킷 16
...
- QA 문서: `docs/qa/V122_V3_GROUNDING_PROFILES_16_2026-08-05.md`

## 2. 목표와 범위

`spore_doll` 한 캐릭터의 4×4 투명 런타임 시트에서 idle/down 셀 발 앵커를 측정하고, 기존 `DataRegistry`의 `normal_grounded` motion entry 병합과 `Unit.gd`의 AtlasTexture·VisualBody/root 기준이 그 값을 소비하는지 직접 검증했다.
...
- `data/v122/combat_visual_profiles.json`
  - `spore_doll` override에 `[192,192]` 프레임 크기와 idle/down 앵커를 추가했다.
  - idle `(0,0)` bbox `[46,35,160,170]`, anchor `[0.5,0.885417]`를 기록했다.
  - down `(2,0)` bbox `[21,87,176,177]`, anchor `[0.5,0.921875]`를 기록했다.
...
- `tools/tests/V122CombatVisualRuntimeProfileContractTest.gd`
  - 이번 패킷 대상을 `spore_doll` 하나로 잠그고 위 bbox·앵커 계약을 검증하도록 갱신했다.
- `docs/qa/V122_V3_GROUNDING_PROFILES_16_2026-08-05.md`
- `docs/handoff/CURRENT.md`

## 4. 자산과 연결 상태

- 기존 런타임 PNG: `res://assets/sprites/enemies/update4/region/normalized/enemy_spore_doll_sheet.png`
...
| JSON·실제 PNG 측정 | spore_doll 4×4 idle/down bbox 및 앵커 비교 Python 검사 | `V122_V3_SPORE_DOLL_PROFILE_JSON_TEST: PASS` |
...
2. 다음 단일 패킷은 `V3-GROUNDING-PROFILES-17`로 제안하며, `root_tender` 한 캐릭터만 동일한 4×4 idle/down 측정·연결 계약을 처리한다.
