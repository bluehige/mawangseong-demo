...
- 첫 `main@1e4b86c1c9082693fd63424dd7a3190a09a15438` Full은 158개 중 154개 PASS, 4개 FAIL이었다.
- 실패 4건은 Stage 02~04의 새 `stage_atlas`, 변경된 `집중 성장` 문구, DAY 3 보조 밸런스 경계의 물리 시뮬레이션 한 틱 오차를 기존 검증 규칙이 따라가지 못한 문제였다. 제품 런타임 결함은 아니었다.
- 검증 커밋 `9deece7501fa11def2f9bbb0c9e256f747042953`에서 해당 4종을 직접 재실행해 모두 PASS했다. 제품 코드·데이터·자산 변경은 없다.
- 다음 순서: 문서 커밋·게이트 수정 PR merge → 최종 `main` SHA Full 1회 → 같은 SHA의 Windows/Web 빌드 → `v1.2.5` Release → Pages 직접 플레이 게시·공개 부팅.
...
- 릴리스 브랜치 문서 PR용 하위 범위 핸드오프: `docs/handoff/V122_RELEASE_CLOSEOUT_REVIEW_2026-08-06.md`
...
- 같은 검수 작업이 `release/v1.2.2@8cc14fa..7c9d135` 문서 마감 하위 범위도 P1/P2/P3 0건, PASS로 별도 확정했다.
