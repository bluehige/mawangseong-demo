...
현재 게임이 사용하는 WAV 50개를 Lyria 3 후보로 다시 만들기 위한 로컬 파이프라인이다. 전투 BGM은 `lyria-3-pro-preview`, 짧은 전투·스킬·경고·모티프 소스 릴은 `lyria-3-clip-preview`를 사용한다. 요청은 모두 Gemini Interactions API의 `interactions.create`로 전송한다.
...
7. 승격 시 선택한 원본, 프롬프트, 모델, Interaction ID, 생성일, SHA-256과 후처리 계약을 `assets/source/audio/lyria/v0.5/<asset>/`에 남긴다.
...
기본 계획은 현재 50개 자산마다 후보 2개를 만드는 구성이다.

| 모델 | 요청 수 | 현재 공식 단가 | 소계 |
|---|---:|---:|---:|
| `lyria-3-clip-preview` | 98 | USD 0.04 | USD 3.92 |
| `lyria-3-pro-preview` | 2 | USD 0.08 | USD 0.16 |
| 합계 | 100 |  | 약 USD 4.08 |

한 테이크씩만 만들면 약 USD 2.04다. 단가는 바뀔 수 있으므로 실제 실행 직전에 공식 가격 페이지와 `plan` 출력을 다시 확인한다.
