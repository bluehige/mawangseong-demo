...
| 정확한 출시본 | `v1.2.0`~`v1.2.5`, `v2.0.0` | 이동하지 않는 출시 스냅샷 | 영구 |
| 버전 통합 | `codex/v125-release-candidate`, 이후 새 `release/v2.0` | 한 출시선을 모아 검수하는 브랜치 | 출시 후 동결 또는 유지보수 |
| 구현 작업 | `codex/v125-<topic>`, 이후 `codex/v20-monsters` | 한 가지 기능, 데이터 또는 자산 작업 | 병합 후 삭제 가능 |
| 테스트·실험 | `test/v125-<topic>`, `test/v20-web` | 밸런스, Web, UX 실험 | 검증 후 삭제 가능 |
...
### 1.2.4~1.2.5 출시 유지보수 계약

`v1.2.4`는 긴급 플레이 버그 수정 기록으로 보존한다. `1.2.5`는 `v1.2.4` 이후 발견된 그래픽·패배 결산·DAY 29 스토리·UI 완성도 결함을 고치며, `codex/v125-release-candidate`에서 전체 검증을 통과한 뒤 `main` PR에 merge commit으로 통합한다. 기존 태그와 Release는 변경하지 않고, 병합된 `main`에 새 `v1.2.5` 태그를 만든다.
