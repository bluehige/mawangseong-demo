...
| 정확한 출시본 | `v1.2.0`, `v1.2.1`, `v1.2.2`, `v2.0.0` | 이동하지 않는 출시 스냅샷 | 영구 |
| 버전 통합 | `release/v1.2.2`, 이후 새 `release/v2.0` | 한 출시선을 모아 검수하는 브랜치 | 출시 후 동결 또는 유지보수 |
| 구현 작업 | `codex/v122-combat`, 이후 `codex/v20-monsters` | 한 가지 기능, 데이터 또는 자산 작업 | 병합 후 삭제 가능 |
| 테스트·실험 | `test/v122-balance`, `test/v20-web` | 밸런스, Web, UX 실험 | 검증 후 삭제 가능 |
...
기존 `release/v2.0-product` 또는 `v2.0.0` 제품 이식 지시는 `docs/design/v122/V122_V20_VALIDATED_TRANSPLANT_PLAN.md`가 대체한다. 정식 `2.0` 확장 통합은 `1.2.2`가 `main`에 병합된 뒤 최신 `main`에서 새로 분기한다.
