...
- `build-manifest.json`에는 태그, 전체 SHA, Godot 버전, UTC 생성 시각, 실제 검수 보고서 경로·해시와 각 핵심 파일의 SHA-256 및 바이트 크기를 기록한다.
...
- 같은 변경에 포함된 모든 PNG, JPEG, WebP, GIF 경로를 변경된 `SOURCE.md` 본문에 정확히 적는다. 원본과 런타임 파생본이 여러 개면 각 경로를 모두 기록한다.
...
| 대상 | 보호 규칙 | 필수 체크 |
|---|---|---|
| `main` | PR 병합, 강제 푸시·삭제 금지 | `repository-policy`, `core-verification`, `web-export-smoke` |
| `release/v*` | PR 병합, 강제 푸시·삭제 금지 | `repository-policy`, `core-verification` |
| 태그 `v*` | 삭제·재지정 금지 | 출시 체크리스트 완료 후 생성 |

현재 이 문서 PR의 `repository-policy` 워크플로는 운영 정책 검사기와 두 검사기의 영구 회귀 테스트를 실행한다. 실제 게임 전체 검증인 `core-verification`, `web-export-smoke`와 GitHub Ruleset은 v0.3 통합 검증 단계에서 추가·실행한 뒤 필수 체크로 설정한다. 적용 전에는 문서상 정책일 뿐 GitHub 강제가 아니므로 `docs/handoff/CURRENT.md`에 미적용 상태를 유지한다.

GitHub의 `github-pages` Environment는 배포 브랜치를 `main`으로 제한한다. 워크플로의 `github.ref` 조건도 함께 유지하며, 둘 중 하나만으로 배포 보호가 완료됐다고 보지 않는다.

2026-07-13 현재 GitHub 저장소 설정은 merge commit만 허용하고 squash merge와 rebase merge는 비활성화했다. 이 설정은 검수 기준 SHA와 실제 병합 이력의 관계를 보존한다. Ruleset을 적용하기 전까지는 관리자 직접 푸시를 기술적으로 막지 못하므로 운영상 PR 병합 원칙을 계속 지킨다.
...
정리 순서는 `v.03` 통합, RC1 명칭과 릴리스 문서 정리, 전체 검증, `main` 병합, 위 매핑에 따른 태그 생성, 미커밋 후속 수정의 패치 분리, 브랜치 보호 및 게임 CI 적용 순이다. 세부 진행 상태는 `docs/handoff/CURRENT.md`를 따른다.
