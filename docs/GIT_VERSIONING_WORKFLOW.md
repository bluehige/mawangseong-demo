...
| 정확한 출시본 | `v1.2.0`~`v1.2.6`, `v2.0.0` | 이동하지 않는 출시 스냅샷 | 영구 |
| 버전 통합 | `codex/v126-release-candidate`, 이후 새 `release/v2.0` | 한 출시선을 모아 검수하는 브랜치 | 출시 후 동결 또는 유지보수 |
| 구현 작업 | `codex/v126-<topic>`, 이후 `codex/v20-monsters` | 한 가지 기능, 데이터 또는 자산 작업 | 병합 후 삭제 가능 |
| 테스트·실험 | `test/v126-<topic>`, `test/v20-web` | 밸런스, Web, UX 실험 | 검증 후 삭제 가능 |
...
`release/v1.2.2`는 `main@7ee0b50965dd3944a7ab737c0eca76d2df2a82ad`에서 시작한다. 제품 런타임 기준은 `v1.2.1@c483d135b13cf9771ee43b045ba2c3dde51573ee`, 전투·밸런스 참고 기준은 `7e61cc9762b5c157a52160ce7f13ad0bf0a7d358`, UI 참고 기준은 `5ed1d5f0bd7f5fcb9b887c20d79749b79707dcc1`이다.

기존 `release/v2.0`과 관련 브랜치는 검증 참고선으로만 읽는다. 다음 작업은 금지한다.

- `release/v2.0` 병합
- `release/v2.0` 커밋 범위 cherry-pick
- `GameRoot.gd`, `CombatSceneController.gd` 등 공통 controller의 통째 복사
- 1.2.1 건물·저장·DAY 1~30 구조를 v20 구조로 교체

기존 `release/v2.0-product` 또는 `v2.0.0` 제품 이식 지시는 `docs/design/v122/V122_V20_VALIDATED_TRANSPLANT_PLAN.md`가 대체한다.

### 1.2.3 출시 증빙 패치 계약

`release/v1.2.3`은 `v1.2.2`가 포함된 최신 `main@42d763b2ed35a541e163b05426ebdbcc0221913a`에서 시작한다. 게임 기능과 화면 표시 `1.2`는 유지하고, 정식 검증 카탈로그의 LF 체크아웃과 Git blob 기준 SHA-256만 보강한다. 공개된 `v1.2.2` 태그와 Release 자산은 변경하지 않는다. 정식 `2.0` 확장 통합은 `1.2.3`이 `main`에 병합·출시된 뒤 최신 `main`에서 새로 분기한다.

### 1.2.4~1.2.6 출시 유지보수 계약

`v1.2.4`는 긴급 플레이 버그 수정 기록으로 보존한다. `1.2.5`는 그래픽·패배 결산·DAY 29 스토리·UI 완성도 결함을 고친다. `1.2.6`은 전투 명령 피드백·AI·방 지침·건설 UX·이름표 가독성을 보강하며 `codex/v126-release-candidate`에서 전체 검증을 통과한 뒤 `main` PR에 merge commit으로 통합한다. 기존 태그와 Release는 변경하지 않고, 병합된 `main`의 검증 SHA에 새 `v1.2.6` 태그를 만든다.
...
사용자가 특정 출시의 검수 범위를 명시적으로 최소화하면 최신 사용자 지시를 우선한다. 이 경우 전체 검증을 실행한 것처럼 기록하지 않고, 고정 SHA의 표적 검수·정식 빌드·부팅·manifest 결과와 미실행 범위를 핸드오프에 명시한 `TARGETED_PASS`로 진행할 수 있다.
