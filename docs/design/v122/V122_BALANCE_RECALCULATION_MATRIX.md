...
DAY 20 승리 뒤 Stage 03 9-room, 수호핵, 1차 승급을 입력으로 시작하고 DAY 23부터 두 번째 승급을 적용했다. DAY 21도 후기 fixture를 사용하도록 바로잡아 실제 Stage 03 상태에서 셀렌 지휘가 종료되는지 검증했다.

| DAY | 실제 E_HP | 목표·실측 초 | 복잡도 할인 | 핵심 ledger | 실제 대표 결과 | 판정 |
|---:|---:|---:|---:|---|---|---|
| 21 | 1,719.0 | 54.6 | 0.90 | 왕실 지휘 6회·43.6초·셀렌 격퇴 후 종료 | 승리·왕좌 0·손실 2 | PASS |
| 22 | 1,737.75 | 50.067 | 0.85 | 공병 1·시설 1 방어 | 승리·왕좌 0·손실 1 | PASS |
| 23 | 1,906.5 | 70.6 | 0.85 | 두 번째 승급·공병 시설 방어 | 승리·왕좌 0·손실 1 | PASS |
| 24 | 1,917.25 | 85.333 | 0.89 | 분리된 방패병·공병 창 | 승리·왕좌 0·손실 1 | PASS |
| 25 | 1,863.25 | 93.267 | 0.79 | 돌진 1회/20·함성 6회/21명·30초 | 방어 지시 승리·왕좌 0·손실 1 | PASS |

DAY 25 기본 `ALL_OUT` 표본은 승리하지만 몬스터 3명이 모두 전투 불능이 되어 허용 손실을 넘었다. 제품 raw 수치는 낮추지 않고 `DEFENSE` 대응을 유효 A로 측정해 93.267초, 전투 불능 1로 고정했다. 레온 재전은 선발 747 HP/13초와 레온·지원 1,116.25 HP/80.267초, 예고 3초, 집중 기준 160, 지원 예산 621.25, 목표 압력 260, 회복 창 7초, 최종 위험 0.24를 별도 phase budget으로 사용한다.

## P14 B5 DAY 26~30

B4 DAY 25 승리 뒤 Stage 03, 두 번째 승급, 최종 캠페인 자원을 순차 fixture로 사용했다. DAY 27 승리 뒤에는 Stage 04 11-room/왕좌 2,500 상태를 확인했고, DAY 28의 두 정찰 선택이 DAY 30 편대를 서로 다르게 줄이는 경로도 각각 실전 표본으로 고정했다. DAY 29는 제품 웨이브가 없는 최종 계획 관리일이므로 전투 예산을 만들지 않고 `MANAGEMENT_PASS`로 별도 판정한다.

| DAY | 실제 E_HP | 목표·실측 초 | 복잡도 할인 | 핵심 ledger | 실제 대표 결과 | 판정 |
|---:|---:|---:|---:|---|---|---|
| 26 | 1,894.05 | 61.267 | 0.86 | 왕실 지휘 7회·49.2초·셀렌 격퇴 후 종료 | 승리·왕좌 0·손실 2·시설 방어 | PASS |
| 27 | 2,043.0 | 83.0 | 0.89 | 공병 2/시설 2 방어·승리 뒤 Stage 04 11-room | 승리·왕좌 0·손실 1 | PASS |
| 28 | 2,079.4 | 64.6 | 0.89 | 공성로 정찰·공병 보급 교란 양쪽 선택 보존 | 두 경로 모두 승리·왕좌 0·손실 0 | PASS |
| 29 | 전투 없음 | 관리 전용 | 해당 없음 | 경로·배치·최종 계획 확인 | `MANAGEMENT_PASS` | PASS |
| 30 | 2,731.75 | 76.8 / 71.333~82.267 | 0.72 | 54초 레온·지휘/돌진/함성/최후의 맹세·경로별 편대 감소 | 두 경로 모두 승리·왕좌 0·손실 0~2 | PASS |

DAY 30 기본 웨이브는 선발·조사관 1,398.55 HP/24초, 셀렌·공병 793.2 HP/30초, 공식 레온 540 HP/22.8초의 3단계로 나눴다. 예고 3초, 집중 중단 기준 180, 공병 소환 예산 462, 목표 압력 300, 회복 창 8초, 최종 위험 0.32를 별도 gate로 고정했다. 공성로 정찰 경로는 조사관 1명을 제거해 71.333초·손실 0, 공병 보급 교란 경로는 공병 1명을 제거해 82.267초·손실 2·시설 교란 1회로 끝났다. 둘 다 왕좌 피해 없이 승리했고 최종 계획 확인 상태를 유지했다.
