...
- 신규 `tools/tests/V20AcceptancePhysicalRunner.gd`
- 신규 `tools/tests/V20AcceptancePhysicalRunner.gd.uid`
- 신규 `tools/tests/V20AcceptancePhysicalRunner.tscn`
- 신규 `tools/tests/V20AcceptanceSessionRecorderTest.gd`
- 신규 `tools/tests/V20AcceptanceSessionRecorderTest.gd.uid`
- 신규 `tools/tests/V20AcceptanceSessionRecorderTest.tscn`
- 신규 `scripts/v20/acceptance/V20AcceptanceSessionRecorder.gd`
- 신규 `scripts/v20/acceptance/V20AcceptanceSessionRecorder.gd.uid`
- `scripts/game/GameRoot.gd`의 수용 기록기 시작·입력·상태 전이·전투 결과 checkpoint hunk만
- `scripts/game/ManagementSceneController.gd`의 배치 시도 기록 신호 연결 hunk만
- `scripts/v20/placement/V20PlacementBoard.gd`의 성공·실패 배치 시도 신호 hunk만
- 신규 `tools/playtest/v20/Run-V20PhysicalAcceptance.ps1`
- 신규 `tools/playtest/v20/New-V20AcceptanceKit.ps1`
- 신규 `tools/playtest/v20/Test-V20PlaytestTools.ps1`
- 신규 `tools/playtest/v20/kit/Start-V20Session.ps1`
- 신규 `tools/playtest/v20/kit/Start-Test.cmd`
- 신규 `tools/playtest/v20/kit/README.txt`
- `tools/tests/core_verification_suite.json`
...
- `V20AcceptancePhysicalRunner`는 한 process에서 한 전투만 실행한다. PowerShell orchestrator는 `A/B/C/D × DAY 1~5 × seed 3개 = 60전`과 `A/B × DAY 1~5 × 첫 seed 결정론 재실행 = 10전`을 별도 process 70개로 만든다. PR 4의 후보 20전은 이 분자·분모에 넣지 않는다.
- 물리 runner는 `run_id`, source SHA, DAY, scenario, seed, purpose, 원 run ID·SHA-256, 시설·몬스터 zone/slot, command 입력, 전체 `event_ledger`, 이동·표적 기록, 물리 frame 수, 전투 시간과 결과를 run별 원본 JSON에 원자적으로 쓴다. `TIMEOUT`, `STUCK` 또는 process 비정상 종료는 해당 source SHA의 실패로 남기고 다음 run으로 숨기지 않는다.
- 참가자 기록기는 debug acceptance mode와 명시적 참가자 ID가 함께 있을 때만 켜진다. `FU01`~`FU10`은 `human_first_user`, QA ID는 `human_qa`, `AGENT-*`는 `synthetic_preflight`로 서로 다른 `evidence_kind`를 저장한다. 합성 persona 기록은 사람 표본 분자·분모에 넣지 않는다.
- 참가자 기록기는 title 화면 노출부터 상태별 체류 시간, 첫 배치·방어 시작 시각, 성공·실패 배치 시도, 20초 이상 입력 공백, DAY별 시도와 기존 `v20_evidence` 원본을 checkpoint마다 저장한다. 이름·연락처·입력한 문자·음성은 기록하지 않는다.
- `Start-Test.cmd`는 참가자별 비어 있는 새 session 폴더만 사용한다. launcher process의 `APPDATA`와 `LOCALAPPDATA`만 격리하고, 기존 `v1.2.1` 저장·Release·공개 build는 읽기 검증 외에는 접근하지 않는다.
- launcher는 정상 종료, 30분 종료, crash와 강제 종료 모두에서 부분 원본을 보존하고 고정 다섯 질문과 관찰자 개입 기록을 받아 `<participant_id>_result.zip`을 만든다. ZIP 안에는 `manifest.json`, `session_metrics.json`, `gameplay_events.json`, `combat_events.json`, `feedback.json`, `observer_notes.json`, `runtime.log`를 둔다.
...
6. 고정 Windows ZIP으로 `synthetic_preflight` persona가 title부터 실제 UI를 조작해 dead click, 진행 차단, 기록 누락과 ZIP 회수 실패를 찾는다. 이 결과는 사람 PASS 근거가 아니다.
7. 같은 source SHA에서 숙련 QA 2명의 공식 A/B 성공 20전과 선행 C 패배 4전, 총 24전을 실행한다.
