...
이 디렉터리는 공유 기획 문서의 구 기획 번호별 원문을 수정 없이 보존한다. 디렉터리의 `v0.*` 이름은 2026-07-16 이전 기획 묶음 코드이며 현재 제품 출시 버전이 아니다. 구현 중 생기는 해석, 완료 표기, 예외와 검증 결과는 원문 파일에 덮어쓰지 않고 버전별 감사 문서와 `docs/handoff/`에 기록한다.
...
- 제품 출시 순서: `1.0 → 1.1 → 1.2 → 2.0 → 3.0 → 4.0`
- 구 기획 원문 순서: `v0.1 → v0.2 → v0.3 → v0.4 → v0.5 → v0.6`
- 단계 게이트: 각 버전의 Phase를 번호순으로 진행하며, 해당 Phase 완료 조건과 직접 관련 검사가 통과하기 전 다음 Phase를 시작하지 않는다.

## 제품 버전 매핑

| 구 기획·출시 표기 | 현재 제품 버전 의미 |
|---|---|
| `v0.1` | 1.0 이전 기반 기획 기록 |
| `v0.2` | 1.0 출시 기준 기획 |
| `v0.2.1` | 1.1 수정판에 대응하는 과거 작업 기록 |
| 현재 지시 전용 전투·UI·입력 수정 | 1.2 현재 수정판 |
| `v0.3` | 2.0 확장판 기획 원본 |
| `v0.4` | 3.0 확장판 기획 원본 |
| `v0.5` | 4.0 확장판 기획 원본 |
| `v0.6` | 후속 기획 원본, 제품 출시 번호 추후 확정 |

정식 기준은 `docs/PRODUCT_VERSIONING.md`다. 기존 파일·폴더·태그 이름은 원문 및 공개 이력 보존을 위해 바꾸지 않는다.
...
1. 안정 버전의 정확한 상태는 이동하지 않는 SemVer 태그(`v1.2.0`, `v2.0.0`)로 고정한다.
2. 다음 버전은 최신 `main`에서 `release/v1.2`, `release/v2.0` 같은 통합 브랜치를 만들고, 개별 Phase는 `codex/v12-<topic>`, `codex/v20-<topic>`에서 진행한다.
