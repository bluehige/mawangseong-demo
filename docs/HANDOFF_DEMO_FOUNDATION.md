...
- 이후 피드백을 반영해 방별 사각형 벽 표현을 제거하고, floor cell을 먼저 만들고 외곽 wall cell을 자동 생성하는 방식으로 고쳤다. 이제 입구에서 가시 복도, 중앙 통로, 왕좌의 방으로 이어지는 주 경로와 분기 방 경로가 타일로 연결되어 보인다.
- 벽 셀에는 돌면, 그림자, 균열, 문턱, 기둥, 횃불을 추가했다. 현재는 실제 던전처럼 보이게 하기 위한 임시 렌더링이며, 최종 타일셋 제작 전 단계다.
...
2. 방별 소품 밀도, 벽 코너, 문턱, 계단, 빈 배경 타일의 미술 품질 추가 보정
...
- 던전은 참고 이미지 방향을 반영한 1차 타일형 구조와 벽 장식까지 보강했지만, 최종 아트 품질의 전용 타일셋/벽 코너/방 장식 세트까지 완성한 단계는 아니다.
