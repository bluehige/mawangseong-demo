...
현재 제품 표시 버전은 `1.2`, 기술 SemVer는 `1.2.2`입니다. 최신 공개 안정판은 `1.2.2`이며 `v1.2.0`·`v1.2.1`·`v1.2.2` 태그는 이동하지 않는 출시 기준입니다. 정식 Web/Windows 빌드는 같은 버전의 GitHub Release에 보관하고, 다음 확장 통합은 최신 `main`에서 새로 시작합니다.
