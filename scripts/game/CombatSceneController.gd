...
func try_attack(attacker: Node, opponents: Array) -> void:
	...
	if target == null and attacker.threat_unit != null and is_instance_valid(attacker.threat_unit) and attacker.threat_unit.is_alive() and opponents.has(attacker.threat_unit) and attacker.global_position.distance_to(attacker.threat_unit.global_position) <= attacker.attack_range:
		target = attacker.threat_unit
...
func use_selected_skill(slot: int) -> bool:
	...
	match skill_id:
		...
		"cleansing_bloom":
			...
			var cleansed := 0
			for ally in root.monster_units:
				if ally.is_alive() and ally.current_room == root.selected_unit.current_room:
					ally.heal(24)
		...
		"stone_pulse":
			...
			for enemy in root.enemy_units:
				if enemy.is_alive() and root.selected_unit.global_position.distance_to(enemy.global_position) <= 180.0:
					var hp_before := int(enemy.hp)
					var dealt = enemy.receive_damage(32)
		...
		"moon_mark":
			...
			var moon_dealt = moon_target.receive_magic_damage(moon_damage)
		...
		"scent_pursuit":
			...
			var scent_dealt = scent_target.receive_damage(scent_damage)
		...
		"false_treasure":
			var lured := 0
			for enemy in root.enemy_units:
				if enemy.is_alive() and root.selected_unit.global_position.distance_to(enemy.global_position) <= 250.0:
